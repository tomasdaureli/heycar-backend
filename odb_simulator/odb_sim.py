    ...
    if status == "OK":
        ...
        for error in odb_codes:
            if component.lower() in error["message"].lower():
                ...
                return {
                    "obd_code": error["obd_code"],
                    "translated_message": translated_message,
                }
        return {
            "obd_code": "CODE_NOT_FOUND",
            "translated_message": "Código No Encontrado",
        }
    else:
        return {"obd_code": "N/A", "translated_message": "No Aplica"}
...
def generate_status():
    data = {
        ...
        "oil_status": {**get_obd_code("oil"), "status": random.choice(status_options)},
        "temperature_status": {
    ...
    while True:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Error al enviar: {response.status_code}, Response: {response.text}"
                )
