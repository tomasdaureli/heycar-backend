...
class UserService:
    ...
    def assign_badge(self,user_id: int, badge_name: str, description: str = None, score: int = 0):
        badge = self.db.query(Badge).filter(Badge.user_id == user_id, Badge.name == badge_name).first()
        if badge:
            ...
            badge = Badge(
                user_id=user_id,
                name=badge_name,
                description=description or f"Otorgado por realizar la acción {badge_name}",
                score=score
            )
            self.db.add(badge)
